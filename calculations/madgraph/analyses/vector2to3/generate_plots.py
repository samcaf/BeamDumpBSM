...
def main():
    ...
    process_dir = '../../processes/vector2to3/'
    num_runs = 5
    ...
    # vector masses
    labels = {'run_01': r'$m_V=0.001$ GeV',
              'run_02': r'$m_V=0.01$ GeV',
              'run_03': r'$m_V=0.1$ GeV',
              'run_04': r'$m_V=1$ GeV',
              'run_05': r'$m_V=10$ GeV'}
    ...
    for xsec, run in zip(xsecs, run_names):
        ...
        for event in events:
            p_v = select_ID(event, [V_PID])
            assert len(p_v) > 0, 'No V found in event'
            assert len(p_v) == 1, 'More than one v in the event'
            p_v = p_v[0]

            costhetas[run].append(cos_theta(p_v))
            xs[run].append(p_v[3]/beam_energies[run])
    ...
    ax_sigma.set_xlabel(r'$m_V$', fontsize=16)
