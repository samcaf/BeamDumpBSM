...
def main():
    ...
    process_dir = '../../processes/pseudoscalar2to3/'
    num_runs = 5
    ...
    # pseudoscalar masses
    labels = {'run_01': r'$m_\phi=0.001$ GeV',
              'run_02': r'$m_\phi=0.01$ GeV',
              'run_03': r'$m_\phi=0.1$ GeV',
              'run_04': r'$m_\phi=1$ GeV',
              'run_05': r'$m_\phi=10$ GeV'}
    ...
    for xsec, run in zip(xsecs, run_names):
        ...
        for event in events:
            p_phi = select_ID(event, [PHI_PID])
            assert len(p_phi) > 0, 'No phi found in event'
            assert len(p_phi) == 1, 'More than one phi in the event'
            p_phi = p_phi[0]

            costhetas[run].append(cos_theta(p_phi))
            xs[run].append(p_phi[3]/beam_energies[run])
    ...
    ax_sigma.set_xlabel(r'$m_\phi$', fontsize=16)
