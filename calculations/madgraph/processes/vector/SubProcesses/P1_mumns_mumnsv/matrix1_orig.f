...
C     ----------
     ...
     $      THEN
            ...
            IF(POL(1).NE.1D0.AND.NHEL(1,I).EQ.INT(SIGN(1D0,POL(1))))
     $        THEN
              T=T*ABS(POL(1))
            ELSE IF(POL(1).NE.1D0)THEN
              T=T*(2D0-ABS(POL(1)))
            ENDIF
...
C        in a common block defined in genps.inc.
        ...
        T=MATRIX1(P ,NHEL(1,I),JC(1),I)
          ...
          ! handling only one beam polarization here. Second beam can be handle via the pdf.
        IF(POL(1).NE.1D0.AND.NHEL(1,I).EQ.INT(SIGN(1D0,POL(1)))) THEN
          T=T*ABS(POL(1))
        ELSE IF(POL(1).NE.1D0)THEN
          T=T*(2D0-ABS(POL(1)))
        ENDIF
...
C     ----------
     ...
     $   *SMALL_WIDTH_TREATMENT)), ZERO)
      ...
      CALL VXXXXX(P(0,5),MDL_MV,NHEL(5),+1*IC(5),W(1,5))
      CALL FFV1_2(W(1,1),W(1,5),GC_7,MDL_MMU, FK_ZERO,W(1,6))
      CALL FORMFACTOR_VSS1P0_1(W(1,2),W(1,4),GC_1,ZERO, FK_ZERO,W(1,7))
C     Amplitude(s) for diagram number 1
      CALL FFV1_0(W(1,6),W(1,3),W(1,7),GC_1,AMP(1))
      CALL FFV1_1(W(1,3),W(1,5),GC_7,MDL_MMU, FK_ZERO,W(1,6))
C     Amplitude(s) for diagram number 2
      CALL FFV1_0(W(1,1),W(1,6),W(1,7),GC_1,AMP(2))
