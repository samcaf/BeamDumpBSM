...
C     
C     Process: mu- ns > mu- ns phi WEIGHTED<=3 @1
C     
      IMPLICIT NONE
C     
C     CONSTANTS
...
C     2 means approximation by the	denominator of the propagator
      ...
      DATA (NHEL(I,1),I=1,5) / 1, 0,-1, 0, 0/
      DATA (NHEL(I,2),I=1,5) / 1, 0, 1, 0, 0/
      DATA (NHEL(I,3),I=1,5) /-1, 0,-1, 0, 0/
      DATA (NHEL(I,4),I=1,5) /-1, 0, 1, 0, 0/
      DATA IDEN/ 2/
...
C     ----------
     ...
     $   *small_width_treatment)), ZERO)
      ...
      AMP(:,:) = (0d0,0d0)
            CALL IXXXXX(P(0,1),MDL_MMU,+1,+1*IC(1),W(1,1))  !  count 3
      CALL IXXXXX(P(0,1),MDL_MMU,-1,+1*IC(1),W(1,2))  !  count 3
      CALL SXXXXX(P(0,2),-1*IC(2),W(1,3))  !  count 1
      CALL OXXXXX(P(0,3),MDL_MMU,+1,+1*IC(3),W(1,4))  !  count 3
      CALL OXXXXX(P(0,3),MDL_MMU,-1,+1*IC(3),W(1,5))  !  count 3
      CALL SXXXXX(P(0,4),+1*IC(4),W(1,6))  !  count 1
      CALL SXXXXX(P(0,5),+1*IC(5),W(1,7))  !  count 4
      CALL FFS1_2(W(1,1),W(1,7),GC_9,MDL_MMU,FK_ZERO,W(1,8))  !  count 2
      CALL FFS1_2(W(1,2),W(1,7),GC_9,MDL_MMU,FK_ZERO,W(1,9))  !  count 2
      CALL FORMFACTOR_VSS1P0_1(W(1,3),W(1,6),GC_1,ZERO,FK_ZERO,W(1,10))  !  count 8
      CALL FFV1P1N_1(W(1,4), W(1,10), GC_1, TMP(1))
      call CombineAmp(2,
     & (/2,4/), 
     & (/8,9/),
     & TMP, W, AMP(1,1))
     ...
     & TMP, W, AMP(1,1))  !  count 1
      CALL FFS1_1(W(1,4),W(1,7),GC_9,MDL_MMU,FK_ZERO,W(1,11))  !  count 2
      CALL FFS1_1(W(1,5),W(1,7),GC_9,MDL_MMU,FK_ZERO,W(1,12))  !  count 2
      CALL FFV1P1N_1(W(1,11), W(1,10), GC_1, TMP(1))
      call CombineAmp(2,
     & (/2,4/), 
     & (/1,2/),
     & TMP, W, AMP(1,2))
      CALL FFV1P1N_1(W(1,12), W(1,10), GC_1, TMP(1))
      call CombineAmp(2,
     & (/1,3/), 
     & (/1,2/),
